...
# IMPORTANT: I must use the exact same random seed to reproduce the issue.
random.seed(42)
...
print('LR R2:', r2_score(Yte, lr.predict(Xte_s)))

xgb1 = xgb.XGBRegressor(n_estimators=200, max_depth=6, learning_rate=0.1, random_state=42, tree_method='hist')
xgb1.fit(Xtr_s, Ytr)
print('XGB (hist) R2:', r2_score(Yte, xgb1.predict(Xte_s)))

xgb2 = xgb.XGBRegressor(n_estimators=200, max_depth=6, learning_rate=0.1, random_state=42, tree_method='exact')
xgb2.fit(Xtr_s, Ytr)
print('XGB (exact) R2:', r2_score(Yte, xgb2.predict(Xte_s)))
