...
**Energy convention:** the surrogate predicts **gross demand-side EUI** (P1–P7 + climate only). PV (P8) / BESS (P9) enter EXACTLY ONCE, through the single `objectives.net_energy()` model (zero-export self-consumption) — used consistently by f1, LCC-OC, LCA-B6 and `assess_nze()`. MOO decision variables are **integer level indices** on the true simulated jEPlus ladders (`config.P*_LEVELS`).

**Economic/LCA basis:** 20-yr study period, 8% real discount, elec 0.137 USD/kWh (EVN before-VAT), grid EF 0.6592 kgCO2e/kWh (MONRE 1726/2024), PV yield 1,420 kWh/kWp/yr. All constants live in `pi_hgat/config.py`; sourced registry: `data/jEPlus-LHS/ICAE2026_DataRegistry_P1-P9.xlsx`.'''
...
# 3. Safely insert Timing and External test
new_cells = []
...
# Edge case: if the notebook doesn't end with a markdown section
section_indices.append(len(nb['cells']))

# Iterate and build the new cell list
current_idx = 0
for s in range(len(section_indices) - 1):
    start_idx = section_indices[s]
    end_idx = section_indices[s+1]
    
    # Add everything up to the section header
    while current_idx <= start_idx:
        new_cells.append(nb['cells'][current_idx])
        current_idx += 1
        
    # Start timer for Section (after the header)
    ...
    # Check if this is Section 8
    is_section_8 = "Section 8" in nb['cells'][start_idx]['source'][0]
    
    # Add cells belonging to this section
    while current_idx < end_idx:
        new_cells.append(nb['cells'][current_idx])
        current_idx += 1
        
    # Insert external test at the end of Section 8
    if is_section_8:
        ext_test_code = (
...
# Add any remaining cells if there are any (though current_idx should be equal to len(nb['cells']))
