...
def fix():
    ...
    # Clean out the wrongly injected cells
    cleaned_cells = []
    for cell in nb.cells:
        if cell.cell_type == 'code':
            src = "".join(cell.get('source', []))
            if src.startswith('# FIG 2 (Mock/Setup') or src.startswith('# FIGURE 5, 6, 7') or \
               src.startswith('# FIGURE 3 & 4') or src.startswith('# FIGURE 8') or \
               src.startswith('# FIGURE 9, 10, 11') or src.startswith('# FIGURE 12, 13, 14'):
                continue
...
"""
    ...
    with open('NZEB_PIPELINE_ICAE2026.ipynb', 'w', encoding='utf-8') as f:
        nbformat.write(nb, f)
    print("Fixed and re-injected figure code cells correctly.")
